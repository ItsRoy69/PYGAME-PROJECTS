...
class GameLevels():
    ...
    def quitgame(self):
        pygame.quit()
        sys.exit()
    '''Whether all elements are still'''
    ...
    def restart(self):
        self.level_pointer = 1
        self.start()
    '''Game level switch interface'''
    def switchlevelinterface(self):
        self.level_pointer += 1
        level_switch_label = Label(self.screen, 425, 100, 400, 200)
        ...
        score_label = Label(self.screen, 450, 300, 300, 100)
        score_label.addtext(f'SCORE: {self.score}', 55, self.cfg.FONTPATH['Comic_Kings'], (236, 240, 241))
        replay_btn = Button(self.screen, 120.5, 500, 300, 100, self.replay, (244, 208, 63), (247, 220, 111))
        replay_btn.addtext('PLAY AGAIN', 60, self.cfg.FONTPATH['arfmoochikncheez'], self.cfg.BACKGROUND_COLOR)
        if self.level_pointer <= self.num_levels:
            next_btn = Button(self.screen, 450, 500, 300, 100, self.start, (88, 214, 141), (171, 235, 198))
            next_btn.addtext('NEXT LEVEL', 60, self.cfg.FONTPATH['arfmoochikncheez'], self.cfg.BACKGROUND_COLOR)
        else:
            next_btn = Button(self.screen, 450, 500, 300, 100, self.restart, (88, 214, 141), (171, 235, 198))
            next_btn.addtext('RESTART', 60, self.cfg.FONTPATH['arfmoochikncheez'], self.cfg.BACKGROUND_COLOR)
        quit_btn = Button(self.screen, 800, 500, 300, 100, self.quitgame, (241, 148, 138), (245, 183, 177))
        quit_btn.addtext('QUIT', 60, self.cfg.FONTPATH['arfmoochikncheez'], self.cfg.BACKGROUND_COLOR)
        basic_label = Label(self.screen, self.screen_size[0] - 270, self.screen_size[1] - 20, 300, 100)
        basic_label.addtext('ROY', 60, self.cfg.FONTPATH['arfmoochikncheez'], (113, 125, 126))
        ...
        while True:
            ...
            for component in [replay_btn, next_btn, quit_btn, level_switch_label, score_label, basic_label]:
                component.draw()
            pygame.display.update()
            clock.tick(self.cfg.FPS)
    '''Game failure interface'''
    def failureinterface(self):
        failure_label = Label(self.screen, 425, 100, 400, 200)
        failure_label.addtext('LEVEL FAILED!', 80, self.cfg.FONTPATH['Comic_Kings'], (236, 240, 241))
        score_label = Label(self.screen, 450, 300, 300, 100)
        score_label.addtext(f'SCORE: {self.score}', 55, self.cfg.FONTPATH['Comic_Kings'], (236, 240, 241))
        replay_btn = Button(self.screen, 250, 500, 300, 100, self.start, (244, 208, 63), (247, 220, 111))
        replay_btn.addtext('TRY AGAIN', 60, self.cfg.FONTPATH['arfmoochikncheez'], self.cfg.BACKGROUND_COLOR)
        quit_btn = Button(self.screen, 750, 500, 300, 100, self.quitgame, (241, 148, 138), (245, 183, 177))
        quit_btn.addtext('QUIT', 60, self.cfg.FONTPATH['arfmoochikncheez'], self.cfg.BACKGROUND_COLOR)
        basic_label = Label(self.screen, self.screen_size[0] - 270, self.screen_size[1] - 20, 300, 100)
        basic_label.addtext('ROY', 60, self.cfg.FONTPATH['arfmoochikncheez'], (113, 125, 126))
        clock = pygame.time.Clock()
        while True:
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    ...
                    if replay_btn.selected(): replay_btn.action()
                    if quit_btn.selected(): quit_btn.action()
            for component in [replay_btn, quit_btn, failure_label, score_label, basic_label]:
                component.draw()
            pygame.display.update()
            clock.tick(self.cfg.FPS)
        ...
        while True:
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    ...
                    if quit_btn.selected(): quit_btn.action()
            for component in [replay_btn, resume_btn, quit_btn, pause_label, basic_label]:
                component.draw()
            pygame.display.update()
            clock.tick(self.cfg.FPS)
    '''Start the game'''
    def start(self):
        # Import all game elves
        ...
        while True:
            ...
            # Determine if the pig hit the wooden stake
                ...
                for j in range(len(blocks)):
                    ...
                    if is_collision:
                        ...
                        if abs(block_magnitude_2 - block_magnitude_1) > 2:
                            ...
                            pigs[i].setdead()
            # Determine if the bird hit the wooden stake
                        ...
                        if is_collision:
                            if abs(bird_magnitude_1 - bird_magnitude_2) > 2:
                                if blocks[j] not in blocks_to_remove:
                                    blocks_to_remove.append(blocks[j])
                                    blocks[j].setdestroy()
            # --Determine if a pig hit a pig or a pig hits a wall
            for i in range(len(pigs)):
                pigs[i].move()
                ...
                pigs[i].draw()
            # Determine if the bird hit a pig or if the bird hit a wall
            for i in range(len(birds)):
                ...
                birds[i].draw()
            # Determine if the stake hit the wood pile or the stake hit the wall
